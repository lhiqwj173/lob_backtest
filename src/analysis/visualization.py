...
class BacktestVisualizer:
    ...
    def create_comprehensive_report(self, asset_history: List[Dict], 
        ...
        self._plot_monthly_returns(ax3, df)
        ...
        # 创建交易分析图
        if trades:
            self._create_trade_analysis(trades, output_dir)
    ...
    def _plot_monthly_returns(self, ax, df: pd.DataFrame) -> None:
        """绘制月度收益热力图"""
        # 计算月度收益
        monthly_returns = df['strategy_nav'].resample('M').last().pct_change().dropna()
        ...
        for i, year in enumerate(years):
            ...
            for month_data in year_data.iteritems():
                ...
                returns_matrix[i, month-1] = month_data[1]
        
        # 绘制热力图
        im = ax.imshow(returns_matrix, cmap='RdYlGn', aspect='auto', vmin=-0.1, vmax=0.1)
        ...
        for i in range(len(years)):
            for j in range(12):
                if not np.isnan(returns_matrix[i, j]):
                    text = ax.text(j, i, f'{returns_matrix[i, j]:.1%}',
    ...
    def _create_trade_analysis(self, trades: List, output_dir: str) -> None:
        """创建交易分析图表"""
        if not trades:
            return
